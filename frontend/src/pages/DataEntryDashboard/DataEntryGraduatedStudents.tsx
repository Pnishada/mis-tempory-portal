// src/pages/DataEntryDashboard/DataEntryGraduatedStudents.tsx
...
const GraduatedStudentsPage: React.FC = () => {
  ...
  const [editingStudentId, setEditingStudentId] = useState<number | null>(null); // For inline edit, stores student_id
  ...
  const loadData = async () => {
    try {
      ...
      // These are eligible students who don't have a graduation record yet
  ...
  const startEditEducation = (student: GraduatedStudentListType) => {
    setEditingStudentId(student.student_id); // Use student_id for tracking
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm("Are you sure you want to delete this graduate record?")) {
      try {
        setError(null);
        await deleteGraduatedStudent(id);
        await loadData();
      } catch (err: any) {
        console.error("Error deleting graduated student:", err);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
          <div className="flex-1">
            ...
            <p className="text-gray-600 text-sm sm:text-base">Track post-graduation education and employment of successful graduates</p>
          </div>
          <div className="grid grid-cols-2 gap-4 text-center sm:text-right">
            <div>
              <p className="text-2xl sm:text-3xl font-bold text-emerald-600">
                {stats?.with_workplace || 0}
              </p>
              <p className="text-gray-500 text-sm">Employed</p>
            </div>
            <div>
              <p className="text-2xl sm:text-3xl font-bold text-blue-600">
                {stats?.with_education || 0}
              </p>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-blue-600 mt-1">
                  {stats?.total ? Math.round((stats.with_education / stats.total) * 100) : 0}%
                </p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-green-600 mt-1">
                  {stats?.total ? Math.round((stats.with_workplace / stats.total) * 100) : 0}%
                </p>
              </div>
                      ...
                      {editingStudentId === student.student_id && editFieldType === "education" ? (
                        <div className="space-y-2">
                          <textarea
                            ...
                            rows={3}
                            placeholder="e.g. BSc in Computer Science - University of Colombo (2024)"
                          ...
                          <div className="flex gap-2">
                            <button
                              ...
                              disabled={submitting}
                              className="flex items-center gap-1 px-3 py-1 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700 transition disabled:opacity-50"
                            >
                            ...
                            <button
                              ...
                              disabled={submitting}
                              className="px-3 py-1 bg-gray-200 text-gray-700 rounded-lg text-sm hover:bg-gray-300 transition disabled:opacity-50"
                            >
                              Cancel
                            </button>
                          </div>
                        ...
                        <div className="group relative">
                          ...
                          <button
                            ...
                            className="absolute top-0 right-0 opacity-0 group-hover:opacity-100 transition-opacity p-1 text-blue-600 hover:text-blue-800"
                      ...
                      {editingStudentId === student.student_id && editFieldType === "workplace" ? (
                        ...
                        <div className="group relative">
                          ...
                          <button
                            ...
                            className="absolute top-0 right-0 opacity-0 group-hover:opacity-100 transition-opacity p-1 text-green-600 hover:text-green-800"
      ...
      {/* Modal for Education/Workplace Details */}
      {openModal && currentStudent && modalType && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-bold text-gray-900">
                  {modalType === "education" ? "Add Graduate Education" : "Add Workplace Details"}
                </h2>
                <button
                  onClick={() => {
                    setOpenModal(false);
              ...
              <div className="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
                <div className="flex items-center mb-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400">{currentStudent.course_name} • {currentStudent.center_name}</p>
                  </div>
                </div>
              </div>
              ...
              <form onSubmit={(e) => { e.preventDefault(); handleSubmit(); }} className="space-y-6">
                {modalType === "education" ? (
                  <div>
                    ...
                    <textarea
                      ...
                      rows={4}
                      placeholder="e.g. BSc in Computer Science - University of Colombo (2024), Diploma in Software Engineering - SLIIT (2023)..."
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                      disabled={submitting}
                    />
                    <p className="text-xs text-gray-500 mt-1">
                      Enter degree/diploma, institution, and year of completion
                    </p>
                  </div>
                ) : (
                  <>
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">
                        Job Role / Description (Optional)
                      </label>
                      <textarea
                        value={jobDescription}
                        onChange={(e) => setJobDescription(e.target.value)}
                        rows={3}
                        placeholder="e.g. Full-Stack Developer, responsible for building web applications..."
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                        disabled={submitting}
                      />
                    </div>
                ...
                <div className="flex gap-3 pt-4">
                  <button
                    ...
                    disabled={submitting}
                    className="flex-1 py-2 bg-gray-100 text-gray-700 font-medium rounded-lg hover:bg-gray-200 transition disabled:opacity-50"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    disabled={submitting}
                    className="flex-1 py-2 bg-emerald-600 text-white font-medium rounded-lg hover:bg-emerald-700 transition disabled:opacity-50 flex items-center justify-center gap-2"
                  >
                    {submitting && <Loader2 className="w-4 h-4 animate-spin" />}
                    {modalType === "education" ? "Save Education Details" : "Save Workplace Details"}
                  </button>
                </div>
              </form>
            </div>
